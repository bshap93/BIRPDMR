...
#if ODIN_INSPECTOR
        private const bool hasOdin = true;
#else
...
#pragma warning restore CS0612 // Type or member is obsolete
            ...
            if (CompilationPipeline.codeOptimization != CodeOptimization.Release)
                HotReloadSuggestionsHelper.SetSuggestionInactive(HotReloadSuggestionKind
                    .SwitchToDebugModeForInlinedMethods);
#endif
        }

        private static void ResetSettingsOnQuit()
        {
            quitting = true;
            ResetSettings();
        }
        ...
        private static async Task FlushErrors()
        {
            ...
            if (response == null || disableServerLogs) return;
...
#if UNITY_2020_1_OR_NEWER
                if (!disableInlineChecks) CheckInlinedMethods();
#endif
            }
        ...
        internal static HashSet<MethodBase> inlinedMethodsFound = new();
        internal static void CheckInlinedMethods()
        {
            ...
            try
            {
                ...
                {
                    if (inlinedMethodsFound.Contains(method)) continue;
                    var isMethodSynthesized = method.Name.Contains("<") ||
                                              (method.DeclaringType?.Name.Contains("<") == true &&
                                               method.Name == ".ctor");
                    if (!(method is ConstructorInfo) && !isMethodSynthesized && MethodUtils.IsMethodInlined(method))
                    {
                        if (newInlinedMethods == null) newInlinedMethods = new HashSet<MethodBase>();
                ...
                if (newInlinedMethods?.Count > 0)
                {
                    ...
                    {
                        Log.Warning(
                            "Unity Editor inlines simple methods when it's in \"Release\" mode, which Hot Reload cannot patch.\n\nSwitch to Debug mode to avoid this problem, or let Hot Reload fully recompile Unity when this issue occurs.");
                        HotReloadPrefs.LoggedInlinedMethodsDialogue = true;
                    }

                    HotReloadTimelineHelper.CreateInlinedMethodsEntry(entryType: EntryType.Foldout,
                        patchedMethodsDisplayNames: newInlinedMethods
                            .Select(mb => $"{mb.DeclaringType?.Name}::{mb.Name}").ToArray());
                    ...
                    HotReloadSuggestionsHelper.SetSuggestionActive(HotReloadSuggestionKind
                        .SwitchToDebugModeForInlinedMethods);
                    foreach (var newInlinedMethod in newInlinedMethods) inlinedMethodsFound.Add(newInlinedMethod);
                    RequestHelper.RequestEditorEventWithRetry(new Stat(StatSource.Client, StatLevel.Debug,
                        StatFeature.Patching, StatEventType.Inlined)).Forget();
                }
            }
            catch (Exception e)
            {
                Log.Warning(
                    $"Inline method checker ran into an exception. Please contact support with the exception message to investigate the problem. Exception: {e.Message}");
            }
        }
#endif
        ...
        private static void HandleAssetChange(string assetPath)
        {
            ...
            if (assetPath.Contains("UnityDirMonSyncFile") || assetPath.EndsWith("~", StringComparison.Ordinal)) return;
        ...
        private static string ToPath(string assetPath)
        {
            ...
            if (relativePath.StartsWith("..", StringComparison.Ordinal)) relativePath = null;
            if (relativePathPackages.StartsWith("..", StringComparison.Ordinal))
            {
                ...
                try
                {
                    foreach (var package in PackageInfo.GetAllRegisteredPackages())
                        ...
                        {
                            relativePathPackages =
                                $"Packages/{package.name}/{assetPath.Substring(package.resolvedPath.Length)}";
                            break;
                        }
                }
                catch
        ...
        private static void HandleResponseReceived(MethodPatchResponse response)
        {
            ...
            partiallySupportedChangesFiltered.RemoveAll(
                x => !HotReloadTimelineHelper.GetPartiallySupportedChangePref(x));
            ...
            if (allMethods == null)
                allMethods =
                    response.removedMethod?.Select(m => GetExtendedMethodName(m))
                        .Distinct(StringComparer.OrdinalIgnoreCase) ?? Array.Empty<string>();
            else
                allMethods = allMethods
                    .Concat(response.removedMethod?.Select(m => GetExtendedMethodName(m)) ?? Array.Empty<string>())
                    .Distinct(StringComparer.OrdinalIgnoreCase);

            var allFields = (patchResult?.addedFields.Select(f => GetExtendedFieldName(f)) ?? Array.Empty<string>())
                .Concat(response.alteredFields?.Select(f => GetExtendedFieldName(f))
                    .Distinct(StringComparer.OrdinalIgnoreCase) ?? Array.Empty<string>())
                .Distinct(StringComparer.OrdinalIgnoreCase);
            ...
            if (!_compileError) lastCompileErrorLog = null;
            ...
            if (_compileError)
            {
                ...
                RequestHelper.RequestEditorEventWithRetry(
                    new Stat(StatSource.Client, StatLevel.Debug, StatFeature.Reload, StatEventType.CompileError),
                    new EditorExtraData
                    {
            ...
            else if (_applyingFailed)
            {
                ...
                HotReloadTimelineHelper.CreateReloadFinishedWithWarningsEventEntry(
                    patchedMembersDisplayNames: patchedMembersDisplayNames);
                HotReloadSuggestionsHelper.SetSuggestionsShown(HotReloadSuggestionKind.UnsupportedChanges);
                if (HotReloadPrefs.AutoRecompileUnsupportedChangesImmediately ||
                    InternalEditorUtility.isApplicationActive) TryRecompileUnsupportedChanges();
                RequestHelper.RequestEditorEventWithRetry(
                    new Stat(StatSource.Client, StatLevel.Debug, StatFeature.Reload, StatEventType.Failure),
                    new EditorExtraData
                    {
                        { StatKey.PatchId, response.id }
                    }).Forget();
            ...
            else if (_appliedPartially)
            {
                ...
                if (HotReloadPrefs.AutoRecompileUnsupportedChangesImmediately ||
                    InternalEditorUtility.isApplicationActive) TryRecompileUnsupportedChanges();
                RequestHelper.RequestEditorEventWithRetry(
                    new Stat(StatSource.Client, StatLevel.Debug, StatFeature.Reload, StatEventType.Partial),
                    new EditorExtraData
                    {
                        { StatKey.PatchId, response.id }
                    }).Forget();
            ...
            else if (_appliedUndetected)
            {
                ...
                RequestHelper.RequestEditorEventWithRetry(
                    new Stat(StatSource.Client, StatLevel.Debug, StatFeature.Reload, StatEventType.Undetected),
                    new EditorExtraData
                    {
                        { StatKey.PatchId, response.id }
                    }).Forget();
            ...
            else
            {
                HotReloadTimelineHelper.CreateReloadFinishedEventEntry(
                    patchedMethodsDisplayNames: patchedMembersDisplayNames);
                RequestHelper.RequestEditorEventWithRetry(
                    new Stat(StatSource.Client, StatLevel.Debug, StatFeature.Reload, StatEventType.Finished),
                    new EditorExtraData
                    {
                        { StatKey.PatchId, response.id }
                    }).Forget();
        ...
        private static string GetExtendedFieldName(SField field)
        {
            var typeName = field.declaringType.typeName;
            var simpleTypeIndex = typeName.LastIndexOf(".", StringComparison.Ordinal);
            if (simpleTypeIndex > 0) typeName = typeName.Substring(simpleTypeIndex + 1);
        ...
        private static async Task StartCodePatcher(LoginData loginData = null)
        {
            ...
            try
            {
                ...
                await HotReloadCli.StartAsync(exposeToNetwork, allAssetChanges, disableConsoleWindow, isReleaseMode,
                    detailedErrorReporting, loginData).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
        ...
        internal static async Task StopCodePatcher(bool recompileOnDone = false)
        {
            ...
            try
            {
                ...
                await ThreadUtility.SwitchToMainThread();
                if (recompileOnDone) HotReloadRunTab.Recompile();
                StartupProgress = null;
            }
            catch (Exception ex)
            {
    ...
    internal static class UnityFieldDrawerPatchHelper
    {
        ...
        private static readonly List<VisualElement> childrenToRemove = new();

        internal static void PatchCustom(Rect contentRect, UnityEditor.Editor __instance)
        {
            if (__instance.target) FieldDrawerUtil.DrawFromObject(__instance.target);
        }
        ...
        internal static void PatchFillDefaultInspector(VisualElement container, SerializedObject serializedObject,
        ...
        private static void HideChildren(VisualElement container, SerializedObject serializedObject)
        {
            if (container == null) return;
            childrenToRemove.Clear();
            foreach (var child in container.Children())
            {
                if (!(child is PropertyField propertyField)) continue;
                try
                {
                    if (serializedObject != null && serializedObject.targetObject &&
                        UnityFieldHelper.IsFieldHidden(serializedObject.targetObject.GetType(),
                            serializedObject.FindProperty(propertyField.bindingPath)?.name ?? ""))
            ...
            foreach (var child in childrenToRemove) container.Remove(child);
            childrenToRemove.Clear();
        }
